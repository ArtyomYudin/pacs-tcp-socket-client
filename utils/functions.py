...
def calculate_card_number(card_number: str) -> int:
    """
    Преобразует строковый номер карты формата "A.B" в единое 32-битное число.

    Формат входных данных:
        card_no = "left.right"

    Где:
        left  — старшая часть номера (целое число)
        right — младшая часть номера (целое число)

    Алгоритм:
        1. Строка разделяется по точке.
        2. Обе части преобразуются в целые числа.
        3. Левая часть сдвигается влево на 16 бит (<< 16),
           тем самым занимая старшие 16 бит итогового числа.
        4. Правая часть помещается в младшие 16 бит с помощью
           побитового OR (|).

    По сути, функция упаковывает два 16-битных числа в одно 32-битное:

        | 16 бит left | 16 бит right |

    Это эквивалентно формуле:
        result = left * 65536 + right
    ...
    Пример:
        ...
        left << 16  = 40 * 65536 = 2_621_440
        result      = 2_621_440 | 3602
                    = 2_625_042
    ...
    Назначение:
        Используется для упаковки составного идентификатора
        (например, код контроллера + номер карты) в одно
        числовое значение для хранения или передачи.

    Важно:
        Предполагается, что обе части не превышают 16 бит
        (0–65535). Иначе возможна потеря данных.
    """
    left, right = map(int, card_number.split("."))
